  ...
  factory: (require) => {
    ...
    const IconCpu = () => (
      React.createElement("svg", { width: 12, height: 12, viewBox: "0 0 16 16", fill: "none", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" },
        React.createElement("rect", { x: 3, y: 3, width: 10, height: 10, rx: 2 }),
        React.createElement("path", { d: "M6 1v2M10 1v2M6 13v2M10 13v2M1 6h2M1 10h2M13 6h2M13 10h2" })
      )
    );

    const IconRotateCcw = () => (
      React.createElement("svg", { width: 13, height: 13, viewBox: "0 0 16 16", fill: "none", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" },
        React.createElement("path", { d: "M2 8a6 6 0 1 0 1.5-4" }),
        React.createElement("path", { d: "M2 3v5h5" })
      )
    );

    const IconSpinner = ({ size = 12, color = "currentColor" }) => (
      React.createElement("svg", {
        ...
        strokeWidth: 2,
        strokeLinecap: "round",
        style: { animation: "dsh-spin 0.85s linear infinite", display: "inline-block", flexShrink: 0 }
      },
        React.createElement("circle", { cx: 8, cy: 8, r: 6, strokeOpacity: 0.25 }),
        React.createElement("path", { d: "M14 8a6 6 0 0 0-6-6" })
      )
    );
    ...
    const ThinkingBadge = () => {
      ...
      return React.createElement("span", {
        ...
        React.createElement(IconSpinner, { size: 11, color: "#3b82f6" }),
      ...
      React.createElement("div", {
        ...
        React.createElement("svg", {
          ...
          React.createElement("path", {
            ...
            style: {
              animation: "dsh-soundwave-dance 1.4s ease-in-out infinite reverse",
              transformOrigin: "center",
              opacity: 0.35
            },
            d: "M 10 16 L 60 16 Q 75 16, 85 9 T 105 23 T 125 7 T 145 25 T 165 8 T 185 24 T 205 10 T 225 22 T 245 11 T 265 21 T 285 12 T 305 20 T 325 13 T 345 19 T 365 14 T 385 18 T 405 15 T 425 17 T 450 16 L 590 16"
          }),
          React.createElement("path", {
            stroke: "#3b82f6",
            strokeWidth: 2,
            ...
            strokeLinejoin: "round",
            style: {
              animation: "dsh-soundwave-dance 1.7s ease-in-out infinite, dsh-soundwave-glow 2.2s ease-in-out infinite",
              transformOrigin: "center"
            },
            d: "M 10 16 L 50 16 Q 65 16, 75 6 L 85 26 L 95 7 L 105 25 L 115 5 L 125 27 L 135 7 L 145 25 L 155 8 L 165 24 L 178 9 L 190 23 L 205 10 L 220 22 L 235 11 L 250 21 L 268 12 L 285 20 L 305 13 L 325 19 L 345 14 L 365 18 L 390 15 L 415 17 L 440 15.5 L 465 16.5 L 490 16 L 590 16"
          })
        )
      )
    );

    if (typeof document !== "undefined" && !document.getElementById("dsh-btw-spin-style")) {
      const style = document.createElement("style");
      style.id = "dsh-btw-spin-style";
      style.textContent = `
        @keyframes dsh-spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        @keyframes dsh-soundwave-dance {
          0%, 100% { transform: scaleY(0.65); opacity: 0.8; }
          35% { transform: scaleY(1.15); opacity: 1; }
          70% { transform: scaleY(0.85); opacity: 0.9; }
        }
        @keyframes dsh-soundwave-glow {
          0%, 100% { filter: drop-shadow(0 0 2px rgba(59, 130, 246, 0.55)) drop-shadow(0 0 5px rgba(59, 130, 246, 0.25)); }
          50% { filter: drop-shadow(0 0 4px rgba(59, 130, 246, 0.85)) drop-shadow(0 0 10px rgba(59, 130, 246, 0.45)); }
        }
      `;
        ...
        if (token.startsWith("`") && token.endsWith("`")) {
          ...
          tokens.push(
            React.createElement("strong", {
              key: key++,
              style: { fontWeight: 700, color: "var(--dsw-alias-label-primary, #ffffff)" }
            }, token.slice(2, -2))
          );
        } else if (token.startsWith("*") && token.endsWith("*")) {
          tokens.push(
    ...
    function renderTextChunk(chunk, keyPrefix) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (!trimmed) {
          if (i > 0 && i < lines.length - 1) {
            elements.push(React.createElement("div", { key: `${keyPrefix}-${k++}`, style: { height: 6 } }));
          }
          continue;
        }
        ...
        if (listMatch) {
          ...
          elements.push(
            React.createElement("div", {
              ...
              React.createElement("span", { style: { color: "#3b82f6", fontWeight: "bold", userSelect: "none" } }, "•"),
              React.createElement("div", { style: { flex: 1 } }, renderInlineMarkdown(listMatch[3]))
            )
          );
        } else {
          elements.push(
      ...
      return React.createElement("div", { key: `chunk-${keyPrefix}` }, elements);
    }
    ...
    function renderMarkdownBlocks(markdownText) {
      ...
      while ((match = codeBlockRegex.exec(markdownText)) !== null) {
        if (match.index > lastIndex) {
          const textChunk = markdownText.substring(lastIndex, match.index);
          blocks.push(renderTextChunk(textChunk, blockKey++));
        }
        const lang = match[1];
        const code = match[2];
        blocks.push(
          ...
          }, React.createElement("code", null, code.trim()))
        );
        lastIndex = codeBlockRegex.lastIndex;
      }

      if (lastIndex < markdownText.length) {
        const textChunk = markdownText.substring(lastIndex);
        blocks.push(renderTextChunk(textChunk, blockKey++));
      }
    ...
    function BtwDockedBanner({ sessionId }) {
      ...
      const [models, setModels] = useState([{ id: "auto", name: "Inherit Session Model" }]);
      const [selectedModel, setSelectedModel] = useState("auto");
      const prevCountRef = useRef(0);

      useEffect(() => {
        fetch('/api/dsh-btw/models')
          .then(res => res.json())
          .then(data => {
      ...
      const handleRetry = (e) => {
        ...
        if (currentItem.loading) return;
        setItems(prev => prev.map(i => i.id === currentItem.id ? { ...i, loading: true, answer: "" } : i));
        fetch('/api/dsh-btw/retry', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ sessionId, itemId: currentItem.id })
        }).catch(() => {});
      };
      ...
      return React.createElement("section", {
        style: {
          ...
          margin: "0 auto 8px auto", width: "100%", maxWidth: "var(--dsh-composer-card-max-width, 768px)",
        ...
        React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, minHeight: 24 } },
          React.createElement("button", {
            ...
            React.createElement("span", {
              ...
              style: { fontSize: 13, color: "var(--dsw-alias-label-secondary, #a1a1aa)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, cursor: "default" }
            }, currentItem.question),
            currentItem.loading
              ? React.createElement(ThinkingBadge)
              : React.createElement("span", {
                  style: {
                    display: "inline-flex",
                    alignItems: "center",
          ...
          React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
            React.createElement("div", {
              ...
              React.createElement("span", { style: { display: "inline-flex", color: "#3b82f6" } }, React.createElement(IconCpu)),
              React.createElement("select", {
                value: selectedModel,
                onChange: handleModelChange,
                onClick: (e) => e.stopPropagation(),
                title: "Select Model for BTW Side Questions",
                style: {
            ...
            (currentItem.answer && currentItem.answer.startsWith("Unable to resolve side question:")) &&
              React.createElement("button", {
                type: "button",
                onClick: handleRetry,
                disabled: currentItem.loading,
                title: "Retry query with selected model",
                style: { border: "none", background: "transparent", cursor: "pointer", color: "#fca5a5", padding: 4, display: "inline-flex", alignItems: "center" }
        ...
        !collapsed && React.createElement("div", {
          ...
          ? React.createElement(NeonSoundwave)
          : (currentItem.answer && currentItem.answer.startsWith("Unable to resolve side question:")
              ? React.createElement("div", {
                  style: {
                    color: "#fca5a5",
                  ...
                  React.createElement("span", { style: { flex: 1 } }, currentItem.answer)
                )
              : renderMarkdownBlocks(currentItem.answer)
            )
        )
      );
    }
