window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ThinkingBadge = ({ retryAttempt, maxRetries }) => {
      const [frame, setFrame] = useState(0);
      const isRetrying = typeof retryAttempt === "number" && retryAttempt > 0;
      ...
      const badgeColor = isRetrying ? "#f59e0b" : "#3b82f6";
      const badgeBg = isRetrying ? "rgba(245, 158, 11, 0.15)" : "rgba(59, 130, 246, 0.15)";
      const displayText = isRetrying ? `Retrying (${retryAttempt}/${maxRetries || 5})...` : THINKING_FRAMES[frame];
      ...
      return React.createElement("span", {
        ...
        React.createElement(IconSpinner, { size: 11, color: badgeColor }),
        React.createElement("span", {
          style: {
            display: "inline-block",
            minWidth: isRetrying ? 85 : 64,
            textAlign: "left"
          }
        }, displayText)
      );
    ...
    const QuantumEqualizer = ({ isRetrying }) => {
      const bars = [
        { h: 14, delay: "0.2s" },
        { h: 22, delay: "0.4s" },
        { h: 28, delay: "0.0s" },
      ...
      const barGradient = isRetrying
        ? "linear-gradient(180deg, #fbbf24 0%, #f59e0b 100%)"
        : "linear-gradient(180deg, #60a5fa 0%, #3b82f6 100%)";
      const barShadow = isRetrying
        ? "0 0 8px rgba(245, 158, 11, 0.55)"
        : "0 0 8px rgba(59, 130, 246, 0.55)";
      const animName = isRetrying ? "dsh-equalizer-pulse-amber" : "dsh-equalizer-pulse";
        ...
        bars.map((bar, idx) =>
          React.createElement("div", {
            ...
            style: {
              ...
              boxShadow: barShadow,
              animation: `${animName} 1.15s ease-in-out infinite`,
    ...
    if (typeof document !== "undefined" && !document.getElementById("dsh-btw-spin-style")) {
      ...
      style.textContent = `
        ...
        @keyframes dsh-equalizer-pulse {
          0%, 100% { transform: scaleY(0.28); opacity: 0.45; }
          50% { transform: scaleY(1); opacity: 1; filter: drop-shadow(0 0 6px rgba(59, 130, 246, 0.85)); }
        }
        @keyframes dsh-equalizer-pulse-amber {
          0%, 100% { transform: scaleY(0.28); opacity: 0.45; }
          50% { transform: scaleY(1); opacity: 1; filter: drop-shadow(0 0 6px rgba(245, 158, 11, 0.85)); }
        }
      `;
    ...
    function renderTextChunk(chunk, keyPrefix) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (listMatch) {
          ...
          const marker = listMatch[2].endsWith(".") ? listMatch[2] : "•";
          elements.push(
            React.createElement("div", {
              key: `${keyPrefix}-${k++}`,
              style: {
              ...
              React.createElement("span", {
                style: {
                  ...
                  fontWeight: 600,
                  fontSize: 12,
                  minWidth: marker.length > 1 ? 16 : 10,
                  userSelect: "none",
                  flexShrink: 0
                }
              }, marker),
    ...
    function BtwDockedBanner({ sessionId }) {
      ...
      return React.createElement("section", {
          ...
          React.createElement("button", {
            ...
            currentItem.loading
              ? React.createElement(ThinkingBadge, {
                  ...
                  maxRetries: currentItem.maxRetries
                })
              : React.createElement("span", {
                  style: {
                    display: "inline-flex",
        ...
        !collapsed && React.createElement("div", {
          ...
          ? React.createElement(QuantumEqualizer, {
              isRetrying: Boolean(currentItem.retryAttempt && currentItem.retryAttempt > 0)
            })
          : (currentItem.answer && currentItem.answer.startsWith("Unable to resolve side question:")
              ? React.createElement("div", {
                  style: {
