...
function extractSessionContext(session, question, previousBtwList) {
	...
	// 1. Natural Active Session Dialogue (complete conversation without artificial gaps)
	try {
		if (typeof session.deriveMessages === "function") {
			const messages = session.deriveMessages();
			if (Array.isArray(messages) && messages.length > 0) {
				const dialogue = messages.filter((m) => m.role === "user" || m.role === "assistant");
				const formatted = dialogue.map((m) => {
					const role = m.role === "assistant" ? "Assistant" : "User";
					const text = getMessageText(m);
					return text ? `- ${role}: ${text.length > 2000 ? text.slice(0, 2000) + "..." : text}` : null;
				}).filter(Boolean);
	...
	if (Array.isArray(previousBtwList) && previousBtwList.length > 0) {
		const btwEntries = previousBtwList.map((item) => {
