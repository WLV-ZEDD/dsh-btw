...
function cleanAbortedDraftPrefix(text) {
	...
	// Detect aborted prompt-mirroring fragments fused to a capitalized restart (e.g. "Kanina po'natin 'yung mga pangunahing pakNapag-usapan...")
	const fusedMatch = cleaned.match(/^([^\n]{0,120}?\b[a-z]{2,})([A-Z][a-z]{2,}\b.*)$/s);
...
- NATURAL LANGUAGE QUALITY: When responding in Filipino/Tagalog, use standard, natural, conversational phrasing (such as "napag-usapan natin", "tinalakay", "walang naging pinal na desisyon"). Never use invented or malformed pseudo-words (such as "tinalakasan", "natake", or "karuntan").
...
- Answer in natural, modern language matching the user's language and phrasing.`;
	...
	const isTransient = (err) => {
		...
		return /500|502|503|504|internal_error|server_error|timeout|econnreset|etimedout|fetch failed/i.test(msg);
	};
	...
	try {
		...
		if (isTransient(err)) {
			console.warn(`[dsh-btw] Transient error on '${targetConfig.provider}' (${err?.message || "error"}). Retrying once in 1s...`);
		...
		throw new Error(`Provider '${targetConfig.provider}' returned error (${err?.message || "unknown"}). Switch to another model in the header.`);
	}
}
