...
function extractSessionContext(ctx, session) {
	...
	// Head + Tail Preservation for long sessions (> 30 turns)
	let finalEntries;
	if (turnEntries.length <= 30) {
		...
		finalEntries = [
			...head,
			`  [... ${skipped} middle turns omitted for brevity ...]`,
			...tail
		];
	}

	const sessionHistory = `Session Conversation History (Chronological from Beginning to End, All Turns):\n${finalEntries.join("\n")}`;
