...
const CONTEXT_TRIGGER_REGEX = /\b(natin|tayo|dito|kanina|ito|iyan|yan|iyon|yon|we|us|our|here|this|current|recent|last|task|session|project|workspace|file|code|error|bug|log|step|ginagawa|ginawa|pinag-?uusapan|pinagusapan|napag-?usapan|napagusapan|napag-?uusapan|napaguusapan|buod|paksa|desisyon|decision|status|summary|start|beginning|first|initial|goal|topic|topics|origin|original|purpose|objective|simula|umpisa|unang|una|layunin)\b/i;
...
function extractSessionContext(ctx, session, question, previousBtwList) {
	...
	const isRecapQuery = /\b(recap|summary|buod|paksa|pinag-?uusapan|pinagusapan|napag-?usapan|napagusapan|napag-?uusapan|napaguusapan|desisyon|decision|topics|overview)\b/i.test(question);
	const sections = [];

	// 1. Whole-Session Chronological Turn Ledger (Turns 1 to N)
	let turnEntries = [];
	try {
		const sessionProjections = typeof ctx?.get === "function" ? ctx.get("sessionProjections") : null;
		...
		if (outline?.turns && Array.isArray(outline.turns) && outline.turns.length > 0) {
			turnEntries = outline.turns.map((t) => {
				...
				return `- Turn ${t.turn}: User asked "${p.slice(0, 100)}" -> Outcome: ${r.slice(0, 120)}`;
			});
			...
			for (let i = 0; i < messages.length; i++) {
				...
				if (m.role === "user" && m.source?.kind === "user") {
					...
					turnEntries.push(`- Turn ${turnCount++}: User asked "${p.slice(0, 100)}" -> Outcome: ${r.slice(0, 120)}`);
				}
			}
		}
	} catch (err) {
	...
	if (turnEntries.length > 0) {
		const capped = turnEntries.length > 35 ? turnEntries.slice(-35) : turnEntries;
		sections.push(`Session Conversation History (Chronological from Beginning to End, All Turns):\n${capped.join("\n")}`);
	}

	// 2. High-Fidelity Recent Working Context (Only for detailed queries, omitted for whole-session recaps to avoid recency bias)
	if (!isRecapQuery) {
		try {
			if (typeof session.deriveMessages === "function") {
				const messages = session.deriveMessages();
				if (Array.isArray(messages) && messages.length > 0) {
					const realTurns = messages.filter((m) => (m.role === "user" && m.source?.kind === "user") || m.role === "assistant");
					const recentDialogue = realTurns.slice(-4);
					const formatted = recentDialogue.map((m) => {
						const role = m.role === "assistant" ? "Assistant" : "User";
						const text = getMessageText(m).replace(/\s+/g, " ").trim();
						return text ? `- ${role}: ${text.length > 250 ? text.slice(0, 250) + "..." : text}` : null;
					}).filter(Boolean);

					if (formatted.length > 0) {
						sections.push(`Latest Turn Detailed Context (Most Recent Activity):\n${formatted.join("\n")}`);
					}
				}
			}
		} catch {}
	...
	// 3. Prior /btw side queries in this session (last 3 items)
	if (Array.isArray(previousBtwList) && previousBtwList.length > 0) {
		const recentBtw = previousBtwList.slice(-3);
		const btwEntries = recentBtw.map((item) => {
			const q = (item.question || "").trim();
			const a = (item.answer || "").replace(/\s+/g, " ").trim();
			if (!q || !a) return null;
			const shortA = a.length > 120 ? a.slice(0, 120) + "..." : a;
			return `- User asked: "${q}" | /btw answered: "${shortA}"`;
		}).filter(Boolean);

		if (btwEntries.length > 0) {
			sections.push(`Prior /btw Side Queries in this Session:\n${btwEntries.join("\n")}`);
		}
	}

	return { title, workspace, sessionHistory: sections.join("\n\n") };
}
...
async function generateFastAnswer(ctx, sessionOrAgent, sessionId, question, wsName, configModel, previousBtwList) {
	...
	const sessionCtx = extractSessionContext(ctx, session, question, previousBtwList);
...
- Active Workspace: ${sessionCtx.workspace || wsName || "default"}`;
	...
	systemPrompt += `\n\nCRITICAL RULES:
- STRICT BREVITY: Every answer must be ultra-compact, direct to the point, and strictly within 2 to 3 sentences maximum.
- FOR SESSION RECAPS / SUMMARIES / PAST TOPICS ("ano ang pinag-usapan natin", "recap", "summary", "tayo", "kanina", "desisyon", "topics"):
  * Base your answer on the ENTIRE "Session Conversation History" provided above. "Natin / tayo / we / us" refers to this workspace session.
  * NEVER say "Hindi tayo nag-usap kanina", "Ito ang unang tanong mo", "Wala tayong napag-usapan", or any disclaimer claiming no prior conversation occurred.
  * Your summary MUST synthesize the major topics across the WHOLE session starting from Turn 1 to the final turn (do NOT omit early topics such as Turn 1 and Turn 2).
  * Deliver the summary in a single tight paragraph (strictly 2 to 3 sentences maximum), naturally highlighting the key topics and stating the final decision or status (or that no final decision was made).
  * Do NOT use bullet points, numbered lists, sub-lists, or laundry lists of places/items.
  * Do NOT repeat headers or restart your answer.
- FOR GENERAL QUESTIONS (facts, coding, translation, quick lookups):
  * Answer directly in 1 to 2 sentences without conversational filler, greetings, or pleasantries.
- Answer in the same language as the user's question (e.g. Tagalog, English).`;
...
async function handleSideQuery(ctx, sessionOrAgent, sessionId, question, wsName, configModel, maxItems) {
	...
	// Collect completed previous /btw Q&As for this session in chronological order
	const previousBtwList = history[sessionId]
		.filter((i) => !i.loading && i.answer && !i.answer.startsWith("Unable to resolve"))
		.slice()
		.reverse();
	...
	try {
		const answer = await generateFastAnswer(ctx, sessionOrAgent, sessionId, question, wsName, configModel, previousBtwList);
		...
		webCtx.effect(() => {
			...
			const unregisterRetry = webCtx.webServer.register({
				...
				handler: async (req, res) => {
					...
					// Collect completed previous /btw Q&As for this session (excluding retrying item)
					const previousBtwList = sessionItems
						.filter((i) => i.id !== itemId && !i.loading && i.answer && !i.answer.startsWith("Unable to resolve"))
						.slice()
						.reverse();

					void (async () => {
						try {
							const answer = await generateFastAnswer(ctx, null, sessionId, target.question, "default", config?.model, previousBtwList);
