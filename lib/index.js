...
function extractSessionContext(session, question, previousBtwList) {
	...
	// 1. Natural Active Session Dialogue: take up to recent 10 turns with 600-char cap to prevent proxy payload crashes in long sessions
	try {
		if (typeof session.deriveMessages === "function") {
			...
			if (Array.isArray(messages) && messages.length > 0) {
				...
				const formatted = recentDialogue.map((m) => {
					...
					return text ? `- ${role}: ${text.length > 600 ? text.slice(0, 600) + "..." : text}` : null;
				}).filter(Boolean);

				if (formatted.length > 0) {
					sections.push(`Active Session Dialogue:\n${formatted.join("\n")}`);
				}
			}
		}
	} catch (err) {
	...
	// 2. Prior /btw side queries in this session for seamless follow-up awareness
	if (Array.isArray(previousBtwList) && previousBtwList.length > 0) {
		const recentBtw = previousBtwList.slice(-5);
		const btwEntries = recentBtw.map((item) => {
			const q = (item.question || "").trim();
			const a = (item.answer || "").trim();
			if (!q || !a) return null;
			const shortA = a.length > 300 ? a.slice(0, 300) + "..." : a;
			return `- User asked: ${q}\n  /btw answered: ${shortA}`;
		}).filter(Boolean);
...
async function generateFastAnswer(ctx, sessionOrAgent, sessionId, question, wsName, configModel, previousBtwList) {
	...
	const sessionCtx = extractSessionContext(session, question, previousBtwList);
...
- Active Workspace: ${sessionCtx.workspace || wsName || "default"}`;
	...
	systemPrompt += `\n\nInstructions:
Answer the user's side question directly, accurately, and thoroughly without unnecessary conversational filler or greetings.
Provide as much detail as necessary to fully satisfy the question (use bullet points or code blocks where helpful).
