...
async function generateFastAnswer(ctx, sessionOrAgent, sessionId, question, wsName, configModel, onRetryProgress) {
	if (!ctx.llm) {
...
- Active Workspace: ${sessionCtx.workspace || wsName || "default"}`;
	...
	systemPrompt += `\n\nInstructions:
- Answer the user's side question directly, accurately, and concisely.
- For session inquiries (summaries, past topics, recent actions), synthesize the session history accurately.
- For general questions (facts, pricing, trivia, coding, translations), provide direct, factual answers.
- When presenting multiple topics, steps, or items, use a clean numbered list (1., 2.) or bulleted list. Avoid markdown tables (|---|).
- Use natural, fluent language matching the user's phrasing and language.
- Format responses cleanly using standard Markdown where helpful.`;
	...
	const retryDelays = [1000, 1500, 2000, 2500, 3000];
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		try {
			...
			if (isTransient(err) && attempt < maxRetries) {
				...
				console.warn(`[dsh-btw] Transient error on '${targetConfig.provider}' (${err?.message || "error"}). Retrying in ${backoffMs}ms (attempt ${currentRetry}/${maxRetries})...`);
				if (typeof onRetryProgress === "function") {
					try {
						onRetryProgress(currentRetry, maxRetries);
					} catch {}
				}
...
async function handleSideQuery(ctx, sessionOrAgent, sessionId, question, wsName, configModel, maxItems) {
	...
	try {
		const answer = await generateFastAnswer(
			ctx,
			sessionOrAgent,
			sessionId,
			question,
			...
			configModel,
			(attempt, max) => {
				pendingItem.retryAttempt = attempt;
				pendingItem.maxRetries = max;
				saveHistory(history);
			}
		);
		pendingItem.answer = answer;
		pendingItem.loading = false;
		pendingItem.retryAttempt = 0;
		saveHistory(history);
