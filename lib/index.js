			...
			if (Array.isArray(messages) && messages.length > 0) {
				...
				if (dialogue.length > 0) {
					if (dialogue.length <= 3) {
						// Short session: provide linear dialogue without duplication
						const formatted = dialogue.map((m) => {
							const role = m.role === "assistant" ? "Assistant" : "User";
							const text = getMessageText(m);
							return `- ${role}: ${text.length > 500 ? text.slice(0, 500) + "..." : text}`;
						}).filter(Boolean);
						if (formatted.length > 0) {
							sessionHistory = `Session Dialogue:\n${formatted.join("\n")}`;
						}
					} else {
						// Long session: Bookend extraction (Initial Session Goal + Recent Dialogue)
						const sections = [];
						const firstUserTurn = dialogue.find((m) => m.role === "user");
						if (firstUserTurn) {
							const rawText = getMessageText(firstUserTurn);
							if (rawText) {
								const initialGoal = rawText.length > 1000 ? rawText.slice(0, 1000) + "..." : rawText;
								sections.push(`Initial Session Goal (Turn 1):\n- User: ${initialGoal}`);
							}
						}

						const recentTurns = dialogue.slice(-3);
						const formattedRecent = recentTurns.map((m) => {
							const role = m.role === "assistant" ? "Assistant" : "User";
							const text = getMessageText(m);
							return `- ${role}: ${text.length > 500 ? text.slice(0, 500) + "..." : text}`;
						}).filter(Boolean);
						...
						if (formattedRecent.length > 0) {
							sections.push(`Recent Active Dialogue (Last 3 Turns):\n${formattedRecent.join("\n")}`);
						}

						sessionHistory = sections.join("\n\n");
					}
				}
			}
		}
