...
const CONTEXT_TRIGGER_REGEX = /\b(natin|tayo|dito|kanina|ito|iyan|yan|iyon|yon|we|us|our|here|this|current|recent|last|task|session|project|workspace|file|code|error|bug|log|step|ginagawa|ginawa|pinag-?uusapan|pinagusapan|status|summary|start|beginning|first|initial|goal|topic|origin|original|purpose|objective|simula|umpisa|unang|una|layunin)\b/i;
...
function extractSessionContext(ctx, session, question, previousBtwList) {
	...
	try {
		...
		if (outline?.turns && Array.isArray(outline.turns) && outline.turns.length > 0) {
			turnEntries = outline.turns.map((t) => {
				...
				return `- Turn ${t.turn}: User asked "${p.slice(0, 80)}" -> Outcome: ${r.slice(0, 100)}`;
			});
			...
			for (let i = 0; i < messages.length; i++) {
				...
				if (m.role === "user" && m.source?.kind === "user") {
					...
					turnEntries.push(`- Turn ${turnCount++}: User asked "${p.slice(0, 80)}" -> Outcome: ${r.slice(0, 100)}`);
				}
			}
		}
	} catch (err) {
	...
	if (turnEntries.length > 0) {
		...
		sections.push(`Whole-Session Turn Ledger (All Topics from Beginning to End):\n${capped.join("\n")}`);
	}

	// 2. High-Fidelity Recent Working Dialogue (Immediate last 3 turns)
	try {
		if (typeof session.deriveMessages === "function") {
			...
			if (Array.isArray(messages) && messages.length > 0) {
				...
				const recentDialogue = realTurns.slice(-6);
				...
				if (formatted.length > 0) {
					sections.push(`Immediate Recent Working Dialogue:\n${formatted.join("\n")}`);
				}
			}
		}
	} catch {}
...
async function generateFastAnswer(ctx, sessionOrAgent, sessionId, question, wsName, configModel, previousBtwList) {
	...
	let systemPrompt = `You are DSH BTW, the dedicated fast side-assistant for DeepSeek Harness.
Live Environment:
...
- FOR SESSION RECAPS / SUMMARIES / PAST TOPICS:
  * Provide a single tight paragraph (strictly 2 to 3 sentences) summarizing the main topics covered across the whole session and any final decision or status.
  * Mention the key topics naturally.
